<?php
     ...
     * @since 1.0
     */
    class CTradeUser extends CApplicationComponent implements IWebUser
    {
        ...
        public function login($identity, $duration=0)
        {
            ...
            {
                guest::mergerCustomer($id, $this->getState('guest_ID'));
        ...
        public function updateCookie()
        {
            ...
            if ($cookie && !empty($cookie->value) && ($data = Yii::app()->getSecurityManager()->validateData($cookie->value)) !== false)
            {
                ...
                if (is_array($data) && isset($data[1], $data[2], $data[3]))
                {
                    ...
                    $cookie->value=Yii::app()->getSecurityManager()->hashData(serialize($new));
                    $cookies->add($cookie->name, $cookie);
                }
            }
        }
